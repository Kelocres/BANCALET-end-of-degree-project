...
public class PlantaTexScript : MonoBehaviour
{
    public SO_PlantaTex tipusPlanta;
    ...
    public int numFase;
    private FasePlantaTex faseActual;
    ...
    public void Plantar(SO_SeedItem llavor)
    {
        ...
        diesDeVida = 0;
        //rendPlanta = GetComponentInChildren<Renderer>();
        Renderer[] children = GetComponentsInChildren<Renderer>();
        foreach (Renderer child in children)
        {
            if (tipusPlanta.esArbre && child.gameObject.tag == "plantatexArbre")
                rendPlanta = child;
            else if (!tipusPlanta.esArbre && child.gameObject.tag == "plantatexPlanta")
                rendPlanta = child;
        }

        //Objtindre SO_Planta per al creixement, sprites i demés data
        tipusPlanta = llavor.plantaTex;

        //Activar fasePlanta[0]
        numFase = 0;
        mort = false;
        faseActual = tipusPlanta.fases[numFase];
        rendPlanta.material = faseActual.matSenseCollita;
    ...
    public void NouDia()
    {
        ...
        if (collita != null && maxCollita > 0 && arruixada)
        {
            ...
            if (collitaActual > 0)
            {
                if (faseActual.matAmbCollita != null)
                    rendPlanta.material = faseActual.matAmbCollita;
                else
                ...
                if (faseActual.matSenseCollita != null)
                rendPlanta.material = faseActual.matSenseCollita;
            else
    ...
    private void PosDiaMort()
    {
        ...
        mort = true;
        collitaActual = 0;
        if (tipusPlanta.matMort != null)
            rendPlanta.material = tipusPlanta.matMort;
