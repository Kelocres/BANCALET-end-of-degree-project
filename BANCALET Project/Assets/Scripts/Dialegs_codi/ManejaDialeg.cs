...
public class ManejaDialeg : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (getObjecte != null)
        {
            ...
            next.onClick.AddListener(delegate { MostrarOracioActual(); });
        ...
        {
            canvas_btnDesicions = new Button[3];
            for (int i = 0; i < 3; i++)
                canvas_btnDesicions[i] = getObjectes[i].GetComponent<Button>();
        }
    ...
    public void MostrarOracioActual()
    {
        ...
        if (liniesPersonatge.Count == 0)
        {
            ...
            else if (dialegActual.despresDialegEsCinematica())
            {
                ...
                {
                    FindObjectOfType<PJ_StateManager>().CanviarState_Cinematica(false);
        ...
        LineaDialeg liniaActual = liniesPersonatge.Dequeue();
