...
public class FaceCameraPersonajes : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //Col·locar personatge en la llista de ManejaCamares
        if (!started)
        {
            mc = FindObjectOfType<ManejaCamares>();
            if (mc == null)
            {
                Debug.Log("FaceCameraPersonajes Start() ManejaCamares == null!!!");
                return;
            }
            mc.GetFCPersonajes(this);
        }
    }
    ...
    void OnDestroy()
    {
        ...
        if (mc != null) mc.DestroyFCPersonajes(this);
    }
}
